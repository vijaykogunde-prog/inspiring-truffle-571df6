const {
  P, H1, H2, figParagraph, cellPara, makeTable, citeOrder, REF,
  Document, Packer, Paragraph, TextRun, HeadingLevel, AlignmentType,
  Table, TableRow, TableCell, WidthType, BorderStyle, ShadingType, PageBreak, VerticalAlign,
} = require("./build_manuscript.js");
...
children.push(
  new Paragraph({
    ...
    children: [new TextRun({ text: "REVIEW", bold: true, size: 20, color: "808080" })],
  }),
  new Paragraph({
    spacing: { after: 300 },
    children: [new TextRun({
      text: "Antenatal corticosteroids revisited: agent, interval, and the problem of incomplete and mixed courses",
      bold: true, size: 34, color: "1F3864",
    })],
  }),
  new Paragraph({
    spacing: { after: 60 },
    children: [new TextRun({ text: "Prepared for departmental and educational use, based on a PMID/DOI-verified evidence synthesis for a UK Level 3 neonatal unit", italics: true, size: 20, color: "404040" })],
  }),
  new Paragraph({
    spacing: { after: 400 },
    children: [new TextRun({ text: "Correspondence and author details to be completed by the submitting unit prior to journal submission.", italics: true, size: 18, color: "808080" })],
  }),
);

// ---------------------------------------------------------------------
// Key messages box (as a shaded table)
// ---------------------------------------------------------------------
function bullet(text) {
  return new Paragraph({
    spacing: { after: 90 },
    bullet: { level: 0 },
    children: [new TextRun({ text: "", size: 20 })].concat(require("./build_manuscript.js").runsFrom(text, { size: 20 })),
  });
}

const keyMessagesTable = new Table({
  width: { size: 9350, type: WidthType.DXA },
  rows: [
    new TableRow({
      children: [new TableCell({
        width: { size: 9350, type: WidthType.DXA },
        shading: { type: ShadingType.CLEAR, fill: "EAF1FB" },
        margins: { top: 200, bottom: 200, left: 220, right: 220 },
        children: [
          new Paragraph({ spacing: { after: 140 }, children: [new TextRun({ text: "Key messages", bold: true, size: 24, color: "1F3864" })] }),
          new Paragraph({ spacing: { after: 80 }, children: [new TextRun({ text: "What is already known on this topic", bold: true, size: 20, color: "1F3864" })] }),
          bullet("A single course of antenatal corticosteroids reduces neonatal death, respiratory distress syndrome, and intraventricular haemorrhage after preterm birth, an effect established since Liggins and Howie's original 1972 trial and confirmed across dozens of subsequent trials.@@Liggins1972,McGoldrick2020@@"),
          bullet("Current guidance permits either dexamethasone or betamethasone as first-line agents, and restricts repeat dosing to specific circumstances."),
          new Paragraph({ spacing: { before: 120, after: 80 }, children: [new TextRun({ text: "What this review adds", bold: true, size: 20, color: "1F3864" })] }),
          bullet("Twelve-hourly dexamethasone dosing, often assumed locally to be non-standard, is the regimen preferred by the World Health Organization and the American College of Obstetricians and Gynecologists, and no trial has ever compared dosing intervals for dexamethasone against a neurodevelopmental outcome.@@WHO2022,Oladapo2020@@"),
          ...
          bullet("No published evidence of any design addresses mixed-agent courses, and the clearest harm signal from repeat dosing in the largest trial is confined to children eventually born at term rather than preterm.@@Asztalos2014@@"),
        ],
      })],
    }),
  ],
});
children.push(keyMessagesTable, new Paragraph({ spacing: { after: 300 }, children: [] }));
...
// Methods
// ---------------------------------------------------------------------
children.push(H1("Search strategy and methods"));
children.push(P(
...
// Section 1
// ---------------------------------------------------------------------
children.push(H1("What is actually recommended: agent, dose, and interval"));
children.push(P(
...
// Table 1
children.push(H2("Table 1. Antenatal corticosteroid regimens by source"));
...
// Section 2
// ---------------------------------------------------------------------
children.push(H1("Does dosing interval affect outcome?"));
children.push(P(
...
// Section 3
// ---------------------------------------------------------------------
children.push(H1("Timing relative to delivery: compressed and incomplete courses"));
children.push(P(
...
// Section 4
// ---------------------------------------------------------------------
children.push(H1("Chorioamnionitis"));
children.push(P(
...
// Section 5
// ---------------------------------------------------------------------
children.push(H1("Dexamethasone versus betamethasone"));
children.push(P(
...
// Section 6
// ---------------------------------------------------------------------
children.push(H1("Mixed-agent courses"));
children.push(P(
...
// Section 7
// ---------------------------------------------------------------------
children.push(H1("Long-term outcomes, repeat courses, and exposure without preterm birth"));
children.push(P(
...
// ---------------------------------------------------------------------
children.push(H1("Practice recommendations"));
children.push(H2("Table 2. Summary recommendations and their certainty"));
children.push(makeTable(
...
// ---------------------------------------------------------------------
children.push(H1("Areas for future research"));
children.push(P("Four gaps identified in this review would meaningfully change practice if filled:"));
children.push(bullet("A trial or sufficiently large cohort directly comparing dexamethasone dosing intervals, with neurodevelopmental follow-up to at least eighteen months."));
children.push(bullet("Independent replication of the ASTEROID cerebral palsy signal, ideally through individual patient data meta-analysis across existing dexamethasone-versus-betamethasone trials."));
children.push(bullet("Any systematic data, even observational, on mixed-agent courses, given how plausible the scenario is in routine handover practice."));
children.push(bullet("Individual patient data meta-analysis of chorioamnionitis subgroups across existing placebo-controlled trials, since a dedicated randomised trial in this population is unlikely to be conducted."));
...
// ---------------------------------------------------------------------
// Figures (placed after conclusion, before references, standard journal order)
// ---------------------------------------------------------------------
children.push(new Paragraph({ children: [new PageBreak()] }));
children.push(H1("Figures"));

children.push(...figParagraph(
  "figure1_decision_pathway.png", 576, 529,
...
children.push(...figParagraph(
  "figure2_forest_plot.png", 600, 477,
...
children.push(...figParagraph(
  "figure3_interval_benefit.png", 600, 384,
  "Figure 3. Adjusted odds ratio for a composite adverse neonatal outcome by treatment-to-delivery interval, redrawn from Melamed et al.@@Melamed2015@@ A partial course still reduces risk relative to no treatment, but is not equivalent to a completed, well-timed course."
));
children.push(...figParagraph(
  "figure4_infographic.png", 600, 313,
  "Figure 4. Absolute risk differences per 1000 women treated with a single course of antenatal corticosteroids compared with no treatment, redrawn from the Cochrane review of McGoldrick et al.@@McGoldrick2020@@"
));
...
Packer.toBuffer(doc).then((buf) => {
  const out = "/home/user/inspiring-truffle-571df6/acs-evidence-synthesis/ACS_review_article.docx";
  require("fs").writeFileSync(out, buf);
  console.log("Wrote", out, "with", citeOrder.length, "references cited.");
});
